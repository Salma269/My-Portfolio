...
test.describe('admin CMS login', () => {
  test('accepts configured username and password', async ({ page }, testInfo) => {
    ...
    await expect(page.getByLabel(/Add image URL/i)).toBeVisible();
    await page.getByLabel(/Add image URL/i).fill('/favicon.svg');
    await page.getByRole('button', { name: /Add image/i }).click();
    const firstProjectImage = page.locator('.project-image-card').first();
    await expect(firstProjectImage).toBeVisible();
    await expect(firstProjectImage.getByRole('button', { name: /Cover photo|Make cover/i })).toBeVisible();
    await expect(firstProjectImage.getByRole('button', { name: /Move image up/i })).toBeVisible();
    await expect(firstProjectImage.getByRole('button', { name: /Move image down/i })).toBeVisible();
    await expect(firstProjectImage.getByRole('button', { name: /Delete image/i })).toBeVisible();
