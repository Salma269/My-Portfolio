  ...
  return (
    <div className="project-showcase">
      {orderedProjects.map((project) => {
        ...
        const contentAttrsValue = contentAttrs(locale, project.shortDescription, project.localeStatus);
        return (
          <article className={`project-card reveal ${project.featured ? 'project-card--featured' : ''}`} key={project.slug}>
            ...
            <div className="project-card__body" {...contentAttrsValue}>
              ...
              <Link className="text-link" to={`/projects/${project.slug}`}>{t('actions.readProject')} {locale === 'ar' ? '←' : '→'}</Link>
            </div>
          </article>
        );
      })}
